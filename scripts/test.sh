...
openssl aes-256-cbc -d -k "$ENCRYPTED_PASSWORD" -in pricetaglist.pem.enc -out scripts/deploy_key.pem
if [ $? -eq 0 ] && [ -f scripts/deploy_key.pem ]; then
  ...
  openssl aes-256-cbc -d -md sha256 -k "$ENCRYPTED_PASSWORD" -in pricetaglist.pem.enc -out scripts/deploy_key.pem
  if [ $? -eq 0 ] && [ -f scripts/deploy_key.pem ]; then
    chmod 400 scripts/deploy_key.pem
    ...
    openssl aes-256-cbc -d -salt -k "$ENCRYPTED_PASSWORD" -in pricetaglist.pem.enc -out scripts/deploy_key.pem
    if [ $? -eq 0 ] && [ -f scripts/deploy_key.pem ]; then
      chmod 400 scripts/deploy_key.pem
      ...
      openssl aes-128-cbc -d -k "$ENCRYPTED_PASSWORD" -in pricetaglist.pem.enc -out scripts/deploy_key.pem
      if [ $? -eq 0 ] && [ -f scripts/deploy_key.pem ]; then
        chmod 400 scripts/deploy_key.pem
        ...
        openssl aes-256-cfb -d -k "$ENCRYPTED_PASSWORD" -in pricetaglist.pem.enc -out scripts/deploy_key.pem
        if [ $? -eq 0 ] && [ -f scripts/deploy_key.pem ]; then
          ...
          openssl aes-256-gcm -d -k "$ENCRYPTED_PASSWORD" -in pricetaglist.pem.enc -out scripts/deploy_key.pem
          if [ $? -eq 0 ] && [ -f scripts/deploy_key.pem ]; then
            chmod 400 scripts/deploy_key.pem
